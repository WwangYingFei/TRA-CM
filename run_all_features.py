...
def parse_args():
    parser = argparse.ArgumentParser('Transformer_Click_Model')
    ...
    train_settings.add_argument("--seed", default=114514, type=int)
    ...
    model_settings.add_argument('--rel_combine', default='linear',
                                help='the combination type for relevance')
    ...
    model_settings.add_argument('--pos_embed_size', type=int, default=128,
                                help='size of the position embeddings')
    model_settings.add_argument('--click_embed_size', type=int, default=128,
                                help='size of the click embeddings')
    model_settings.add_argument('--vtype_embed_size', type=int, default=128,
                                help='size of the vtype embeddings')
    model_settings.add_argument('--seq_len', type=int, default=10,
                                help='length of the sequence')
    model_settings.add_argument("--hidden_act", default="relu", type=str) # gelu relu
    model_settings.add_argument("--initializer_range", default=0.02, type=float)
    model_settings.add_argument('--use_attention', type=bool, default=False,
                                help='whether use gru attention')
...
def valid(args, dataset):
    ...
    """
    logger = logging.getLogger("Transformer_Click_Model")
    logger.info('Initialize the model...')
    model = Model(args, dataset.query_size, dataset.doc_size, dataset.vtype_size, dataset)
    logger.info('model.global_step: {}'.format(model.global_step))
    ...
    # print('Loss on valid set: {}'.format(float(valid_loss)))
    # print('Perplexity on valid set: {}'.format(float(perplexity)))
...
def test(args, dataset):
    ...
    # print('Loss on test set: {}'.format(float(test_loss)))
    # print('Perplexity on test set: {}'.format(float(perplexity)))
...
def rank(args, dataset):
    ...
    for trunc_level in trunc_levels:
        ...
        # print("NDCG@{}: {}".format(trunc_level, ndcgs[trunc_level]))
