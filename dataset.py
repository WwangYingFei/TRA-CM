...
class Dataset(object):
    ...
    def __init__(self, args):
        ...
        self.label_set = self.load_dataset(
            os.path.join('data', self.dataset, 'human_label_for_TRACM_per_query_quid.txt'), mode='label')
    ...
    def load_dataset(self, data_path, mode):
        ...
        for line in lines:
            ...
            # print(qid)
            qids = [qid for _ in range(self.max_d_num)]
            query_content = attr[2].strip()
            url_content = attr[3].strip()
            result = eval(url_content)  # 格式还原为列表
            candid_list = []
            for m in result:
                s = query_content + '[SEP]' + m
                candid_list.append(s)
            uids = json.loads(attr[4].strip())
            vids = json.loads(attr[5].strip())
            clicks = [0, 0] + json.loads(attr[6].strip())
            relevances = json.loads(attr[7].strip()) if mode == 'label' else [0 for _ in range(self.max_d_num)]
    ...
    def _one_mini_batch(self, data, indices):
        ...
        for sidx, sample in enumerate(batch_data['raw_data']):
            ...
            candid_token = self.tokenizer(sample['candids'], max_length=128, truncation=True, padding='max_length',
                            return_tensors='pt')
            batch_data['candids'].append(candid_token)
    ...
    def gen_mini_batches(self, set_name, batch_size=64, shuffle=True):
        """
